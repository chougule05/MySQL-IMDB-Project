...
select year, count(*) as count_movies from movie group by year; 

select month(date_published), count(*) as count_movies from movie 
...
select country, count(*) as movies_Count from movie 
where (country = 'India' or country = 'USA') and year = 2019 
group by country;

-- or

SELECT count(distinct id) AS NoOfMovies, year 
FROM movie 
...
select genre, count(*) as movie_Count from genre 
group by genre
...
rank() over(order by avg_rating desc, title ) as movie_rank 
from ratings r
...
SELECT m.languages, sum(r.total_votes) AS votes
FROM movie AS m
...
(
           SELECT n.NAME AS actress_name, sum(total_votes) AS total_votes, Count(r.movie_id)  AS movie_count,
			Round(Sum(avg_rating*total_votes)/Sum(total_votes),2) AS actress_avg_rating FROM movie AS m
           INNER JOIN ratings AS r ON m.id=r.movie_id
           INNER JOIN role_mapping AS rm ON m.id = rm.movie_id
           INNER JOIN names AS n ON rm.name_id = n.id
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
SELECT name, SUM(total_votes) AS total_votes, COUNT(rm.movie_id) AS movie_count, AVG(r.avg_rating) AS avg_rating,
	DENSE_RANK() OVER(ORDER BY AVG(r.avg_rating) DESC) AS actress_rank FROM names AS n
...
GROUP BY name HAVING avg(r.avg_rating) > 8 LIMIT 3;
...
(
 ...
 (
	 ...
	 FROM date_diff GROUP BY name_id
 ), 
 final_answer AS
 (
	 SELECT d.name_id AS director_id, name AS director_name, COUNT(d.movie_id) AS number_of_movies,
		 ROUND(avg_interval_movie_days) AS inter_movie_days, ROUND(AVG(avg_rating),2) AS avg_rating,
	 ...
	 FROM names AS n 
		JOIN director_mapping AS d ON n.id=d.name_id
		 ...
		 JOIN avg_interval_days AS a ON a.name_id=d.name_id
	 GROUP BY director_id
 )
 SELECT * FROM final_answer LIMIT 9;
